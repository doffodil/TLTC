...
def train(model, optimizer, source_data_loader, target_data_loader):
    ...
    for epoch in range(args.EPOCH):
        for batch_num, ((source_input_ids, source_attention_mask, source_label),\
            ...
            optimizer.zero_grad()
            model.train()
            source_predictions, mmd_loss = \
                model(source_input_ids, source_attention_mask, target_input_ids, target_attention_mask)
            ...
            loss = 0.5*cls_loss + 0.5*mmd_loss*lambd
            # loss = mmd_loss *0.2
            # loss = cls_loss
            loss.backward()
            optimizer.step()
            ...
            # ===================================== #
            # eval
            if batch_num != 0 and batch_num % 199 == 0:
                test_acc.append(eval(model, target_data_loader))
            # if batch_num == 20: break
    gc.collect()

    # save loss as file
    loss_lines = {"loss":loss_line, "mmd_loss_line":mmd_loss_line, "cls_loss_line":cls_loss_line}
...
def eval(model, target_data_loader):
    ...
    with torch.no_grad():
        for (target_input_ids, target_attention_mask, target_label) in target_data_loader:
            ...
            predictions, mmd_loss = model(target_input_ids, target_attention_mask, target_input_ids, target_attention_mask)
